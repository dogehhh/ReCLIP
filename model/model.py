...
class RECLIPPP_DISTILL_Head(nn.Module):
    def __init__(self, cfg, cls_name_token, clip_model, text_categories,
                 text_embeddings, text_channels=512, cls_bg=False, norm_feat=True,
                 unlabeled_cats=[], clip_unlabeled_cats=[],
                 reset_counter=False, clip_channels=768,
        ...
        self.pe_proj = nn.Conv2d(768, 512, kernel_size=1)
        self.pe_proj.weight = nn.Parameter(weight['pe_proj.weight'], requires_grad=False)
        self.pe_proj.bias = nn.Parameter(weight['pe_proj.bias'], requires_grad=False)

        self.pe_proj_res = nn.Conv2d(768, 512, kernel_size=1)
        self.pe_proj_res.weight = nn.Parameter(weight['pe_proj.weight'], requires_grad=False)
        self.pe_proj_res.bias = nn.Parameter(weight['pe_proj.bias'], requires_grad=False)

        # decoder
        self.decoder_conv2 = nn.Conv2d(512+self.cnum, self.cnum, kernel_size=5, padding=2, stride=1)
        self.decoder_conv2.weight = nn.Parameter(weight['decoder_conv2.weight'], requires_grad=False)
        self.decoder_conv2.bias = nn.Parameter(weight['decoder_conv2.bias'], requires_grad=False)

        self.decoder_norm2 = nn.BatchNorm2d(self.cnum)
        self.decoder_norm2.weight = nn.Parameter(weight['decoder_norm2.weight'], requires_grad=False)
        self.decoder_norm2.bias = nn.Parameter(weight['decoder_norm2.bias'], requires_grad=False)
        self.decoder_norm2 = self.decoder_norm2.to(device)
    ...
    def cls_seg(self, feat, pe=None, shape=None):
        ...
        pe = F.interpolate(pe.permute(0, 2, 1).reshape(1, -1, shape[0], shape[1]), size=output_q.shape[2:], mode='bilinear', align_corners=False)
        pe = pe.reshape(1, 512, -1).permute(0, 2, 1)
        bias_logits = pe @ self.reference_prompt.t()
        bias_logits = bias_logits.permute(0, 2, 1).reshape(1, self.cnum, output_q.shape[2], output_q.shape[3])
    ...
    def assign_label(self, gt_semantic_seg, feat, norm=True, unlabeled_cats=None,
        ...
        bias_logits = pe @ self.reference_prompt.t()
        bias_logits = bias_logits.permute(0, 2, 1).reshape(1, self.cnum, shape[0], shape[1])
        output = torch.sub(output_q, bias_logits)
    ...
    def forward(self, filenames=None, text=None, train=None, self_train=False, inputs=None, img=None,
        ...
        if train:
            ...
            with torch.no_grad():
                ...
                if self.vit:
                    if isinstance(x, list) and len(x) == 5:
                        ...
                        positional_embedding = positional_embedding.reshape(1, shape[0], shape[1], -1).permute(0, 3, 1, 2)
                        pe = self.pe_proj(positional_embedding).permute(0, 2, 3, 1).reshape(1, shape[0] * shape[1], -1)
                        pe_res = self.pe_proj_res(positional_embedding).permute(0, 2, 3, 1).reshape(1, shape[0] * shape[1], -1)
            ...
            mask = (gt_semantic_seg != 255)
            l1_loss = F.l1_loss(feat.permute(0, 2, 3, 1)[mask], clip_feat.permute(0, 2, 3, 1)[mask])
            l1_loss_logits = F.l1_loss(logits.permute(0, 2, 3, 1)[mask], logits_clip.permute(0, 2, 3, 1)[mask])
            l1_loss = F.l1_loss(feat.permute(0, 2, 3, 1), clip_feat.permute(0, 2, 3, 1))
            l2 = torch.nn.MSELoss()
            l2_loss = l2(feat.permute(0, 2, 3, 1)[mask], clip_feat.permute(0, 2, 3, 1)[mask])
...
class RECLIPPP_DISTILL(nn.Module):
    def __init__(self, clip_model, cfg, cls_name_token, text_categories, text_channels, text_embeddings):
        super().__init__()
        self.backbone = ResNetV1c(cfg)
        self.decode_head = RECLIPPP_DISTILL_Head(clip_model=clip_model, cfg=cfg, cls_name_token=cls_name_token,
                                            text_categories=text_categories, text_embeddings=text_embeddings,
                                            text_channels=text_channels)
